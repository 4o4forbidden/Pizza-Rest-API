class UsersService {
    ...
    async updateUsers (userEmail, data) {
        // Check if user exists
        let _user = await this.userModel.findOne({email: userEmail}).exec();
        console.log(_user);
    ...
    async deleteUser(userEmail) {
        await this.userModel.findOneAndRemove({email: userEmail});

        return {
            message: `User associated with email -> ${userEmail} deleted Successfully!`
        };
    }
